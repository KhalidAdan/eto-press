/**
 * Deterministic classification — the eto engine deciding what counts as
 * news. Experiment 002 failure mode 2: opinion/video/podcast/live items
 * act as glue between unrelated clusters — only `news` items may
 * participate in event matching, and these functions decide which is
 * which with no model involved. This is doctrine, not plumbing, which is
 * why it lives in the engine and is injected into the platform's feed
 * ingest rather than baked into it.
 */
import type { ItemKind } from "@eto-press/platform/normalize"
